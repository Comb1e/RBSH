...
Auto-detection chain: `ipopt → scip`

### Visualization

**`src/visualizer.py`** — `create_dispatch_dashboard()` creates a Dash app and returns it (not just the layout Div). Contains: unit output bar chart (colored by cost), line flow utilization scatter (sized by absolute flow), cost pie chart (top-3 + others), and summary metrics table.
